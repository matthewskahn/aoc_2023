...
lines = list(map(lambda x: x.replace('A', 'E').replace('T', 'A').replace('J', '1').replace('Q', 'C').replace('K', 'D'), lines))
...
def determine_rank(hand: list[int]) -> int:
    cards_by_rank = {}
    for card in hand:
        if cards_by_rank.get(card):
            cards_by_rank[card] += 1
        else:
            cards_by_rank[card] = 1
        
    if cards_by_rank.get(1) and cards_by_rank[1] != 5:
        jokers = cards_by_rank.pop(1)
        max_key = max(cards_by_rank, key=lambda k: cards_by_rank[k])
        cards_by_rank[max_key] += jokers
